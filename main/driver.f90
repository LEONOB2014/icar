...
program icar
    ...
    ! use coarray_module,     only : distribute_initial_data, receive_initial_data              ! pass the initialized data structures to each image
    ...
    print *, "hello from image",image_number
!-----------------------------------------
!  Model Initialization
!
!   initialize model including options, terrain, lat, lon data. 
    
    if (image_number==kMASTER_IMAGE) then
        ...
        ! call distribute_initial_data(domain, boundary, options, next_output)
    ! else
        ! call receive_initial_data(domain, boundary, options, next_output)
    ! endif
